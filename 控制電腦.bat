...
echo.
echo 檢查更新...
getamod http://www.webtop.ml/disk/file/william/update.txt -q
cls
...
set ap=
set /p aq=請輸入命令 [ %cd% ] :~# 
for /f "tokens=1-3 delims= " %%i in ("%aq%") do set ai=%%i && set ai2=%%j && set ai3=%%k
for /f "tokens=*" %%i in ("%ai2%") do set ai2=%%~nxi
set "ai=%ai: =%"
if "%ai%"=="cd" cd %ai2% && goto ai
set cddir=%cd%\%ai%
...
call "%~dp0"mod\addmod.exe %ai2% %ai3%) else (
echo   遺失addmod模組
timeout /t 3
)
cls
